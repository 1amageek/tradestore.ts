...
describe("Manager", () => {
    ...
    describe("orderCancel", async () => {
        test("Success", async () => {
            ...
            const manager: Tradable.Manager<InventoryStock, SKU, OrderItem, Order, TradeTransaction, BalanceTransaction, Payout, User, Account> = new Tradable.Manager(InventoryStock.self(), SKU.self(), Order.self(), TradeTransaction.self(), BalanceTransaction.self(), Payout.self(), User.self(), Account.self())
            ...
            const item = (result.tradeTransactions[0][0].data() as any)["item"]
